...
public class ia_poker {
    ...
    public static void main(String[] args) {
        ...
        ArrayList <Integer> dineroJugadores = new ArrayList<>();
        String ganador = "";
        int posDealer;
        int numJugadores;
        int bote;
        ...
        while (true) {
            iniciar_arrays(manos, palos, valores, baraja, numJugadores, boteApostado, nombreJugador, dineroJugadores, boteInicial);
            if (numJugadores > 2){
                boteApostado.set((posDealer + 1)%boteApostado.size(), ciegaInicial/2);
                dineroJugadores.set((posDealer + 1)%boteApostado.size(), dineroJugadores.get((posDealer + 1)%boteApostado.size()) - ciegaInicial/2);
                boteApostado.set((posDealer + 2)%boteApostado.size(), ciegaInicial);
                dineroJugadores.set((posDealer + 2)%boteApostado.size(), dineroJugadores.get((posDealer + 2)%boteApostado.size()) - ciegaInicial);
            }
            else {
                boteApostado.set((posDealer)%boteApostado.size(), ciegaInicial/2);
                dineroJugadores.set((posDealer)%boteApostado.size(), dineroJugadores.get((posDealer)%boteApostado.size()) - ciegaInicial/2);
                boteApostado.set((posDealer + 1)%boteApostado.size(), ciegaInicial);
                dineroJugadores.set((posDealer + 1)%boteApostado.size(), dineroJugadores.get((posDealer + 1)%boteApostado.size()) - ciegaInicial);
    ...
    // ♣\u2663
            ...
            System.out.println("\nCiegas "+ciegaInicial/2+"/"+ciegaInicial);
            bote = fase_apuestas(ciegaInicial, bote, posDealer, boteApostado, manos, nombreJugador, dineroJugadores);
            ...
            if (boteApostado.size()>1) {
                ...
                bote = fase_apuestas(0, bote, posDealer, boteApostado, manos, nombreJugador, dineroJugadores);
            }
            ...
            if (boteApostado.size()>1) {
                ...
                ganador = comprobar_ganador(manos, mesa, valores, manosPoker, nombreJugador);
                System.out.println("WIN "+ganador);
            }

            ajustarPagos(bote, nombreJugador, ganador);
    ...
    static private void ajustarPagos (int bote, ArrayList <Integer> nombreJugador, String ganador) {
        if (nombreJugador.size() == 1) System.out.println(nombreJugador.get(0)+". + "+bote+"€");
        else System.out.println(ganador+". + "+bote+"€");
    }
    ...
    static private int fase_apuestas (int ciegaInicial, int bote, int posDealer, ArrayList<Integer> boteApostado, ArrayList <String []> manos, ArrayList <Integer> nombreJugador, ArrayList <Integer> dineroJugadores) {
        String opcion = "";
        int apuesta = 0;
        int maxApostado;
        if (ciegaInicial == 0) maxApostado = max_apostado(boteApostado);
            ...
            while(contadorVuelta < boteApostado.size() && (maxApostado != boteApostado.get(i) || primeraVuelta)) {
                if (ciegaInicial != 0) {
                    if (boteApostado.size() > 2){
                        ...
                        if (i == (posDealer) % boteApostado.size()) System.out.print(nombreJugador.get(i)+": \tSB\tc/r/f\t:");
                        else if (i == (posDealer + 1) % boteApostado.size()) System.out.print(nombreJugador.get(i)+": \tGB\tc/r\t:");
                    }
                }
                ...
                switch (opcion) {
                    case "c":
                        if (!apuesta_finalizada(boteApostado) || ciegaInicial != 0){
                            System.out.println("Call "+(maxApostado - boteApostado.get(i)));
                            bote = bote + (maxApostado - boteApostado.get(i)); 
                            dineroJugadores.set(i, dineroJugadores.get(i) - (maxApostado - boteApostado.get(i)));
                    ...
                    case "r":
                        System.out.print(": ");
                        do {
                            apuesta = sc.nextInt() + apostadoFase;sc.nextLine();
                            apostadoFase = apostadoFase + apuesta + ciegaInicial;
                            bote = bote + (maxApostado + apuesta) - boteApostado.get(i);
                            boteApostado.set(i, maxApostado + apuesta);
                            maxApostado = boteApostado.get(i);
                            dineroJugadores.set(i, dineroJugadores.get(i) - apuesta);
                ...
                // }
                // System.out.println("\t");
                // for (int j = 0; j < dineroJugadores.size(); j++) {
                //     System.out.print(dineroJugadores.get(j)+" ");
                // }
                // System.out.println();
    ...
    static private String comprobar_ganador (ArrayList <String []> manos, ArrayList <String> mesa, String [] valores, String [] manosPoker, ArrayList <Integer> nombreJugador) {
        ...
        if (combinaciones.size()>1){ //SI HAY MAS DE UNO CON LA MISMA COMBINACION DE CARTAS
            ...
            else if (!mejorCombinacion.equals("Escalera_real")) {
        ...
        for (int i = 0; i < valores.size(); i++) {
            ...
            if (repetidos.get(i)>1) { //MIRAR QUE VALORES SE REPITEN 
                if (repetidos.get(i)==2){//SI SE ENCUENTRA DOS VECES (PAR)
                    if (!resultado.split(" ")[0].equals("Poker")) { //SI HAY POKER/FULL NO MODIFICAR
                        if (resultado.split(" ")[0].equals("Par")){ //SI YA HAY PAR PONER DOBLE PAR
                            String [] aux = resultado.split(" ");
                            if (esMayorQue(valores.get(i), aux[aux.length-1], valorCartas)) resultado = "Doble par de "+valores.get(i)+" "+valores.get(i)+" "+resultado.split(" ")[resultado.split(" ").length-1]+" "+resultado.split(" ")[resultado.split(" ").length-1];
                            else resultado = "Doble par de "+resultado.split(" ")[resultado.split(" ").length-1]+" "+resultado.split(" ")[resultado.split(" ").length-1]+" "+valores.get(i)+" "+valores.get(i); 
                        }
                        ...
                        else if (resultado.split(" ")[0].equals("Doble")) { // SI YA HAY DOBLE PAR
                            String [] aux = resultado.split(" ");
                            if (esMayorQue(valores.get(i), aux[aux.length-3], valorCartas)) resultado = "Doble par de "+valores.get(i)+" "+valores.get(i)+" "+resultado.split(" ")[resultado.split(" ").length-1]+" "+resultado.split(" ")[resultado.split(" ").length-1];
                            else if (esMayorQue(valores.get(i), aux[aux.length-1], valorCartas)) resultado = "Doble par de "+resultado.split(" ")[resultado.split(" ").length-3]+" "+resultado.split(" ")[resultado.split(" ").length-3]+" "+valores.get(i)+" "+valores.get(i); 
                        }
                        else if (resultado.split(" ")[0].equals("Full")){
                            String [] aux = resultado.split(" ");
                            if (esMayorQue(valores.get(i), aux[aux.length-1], valorCartas)) resultado = "Full de "+aux[aux.length-3]+" "+aux[aux.length-3]+" "+aux[aux.length-3]+" "+valores.get(i)+" "+valores.get(i);
                        }
                        else {
                    ...
                    if (!resultado.split(" ")[0].equals("Poker") && !resultado.split(" ")[0].equals("Full")) { //SI HAY POKER NO MODIFICAR
                        if (resultado.split(" ")[0].equals("Par")) { //SI YA HAY PAR PONER FULL
                            ...
                            if (esMayorQue(resultado.split(" ")[resultado.split(" ").length-1], valores.get(i), valorCartas)) resultado = "Full de "+resultado.split(" ")[resultado.split(" ").length-1]+" "+resultado.split(" ")[resultado.split(" ").length-1]+" "+resultado.split(" ")[resultado.split(" ").length-1]+" "+valores.get(i)+" "+valores.get(i);
                            else resultado = "Full de "+valores.get(i)+" "+valores.get(i)+" "+valores.get(i)+" "+resultado.split(" ")[resultado.split(" ").length-1]+" "+resultado.split(" ")[resultado.split(" ").length-1];  
                        }
    ...
    static void iniciar_arrays (ArrayList <String []> manos, String [] palos, String [] valores, ArrayList<String> baraja, int numJugadores, ArrayList <Integer> boteApostado, ArrayList <Integer> nombreJugador, ArrayList <Integer> dineroJugadores, int boteInicial) {
        ...
        for (int i = 0; i < numJugadores; i++) {
            ...
            dineroJugadores.add(boteInicial);
        }
    }
}
